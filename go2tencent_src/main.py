...
# 定义随机字符串
random_char = ''.join(random.choice(string.ascii_lowercase + string.digits) for x in range(5))
...
def resource_oper(logapp):
    ...
    if cret_vpc_result:
        ...
        print('{}发生异常'.format('创建VPC'))
        logapp.info('{}发生异常'.format('创建VPC'))
        exit(1)
    ...
    if cret_subnet_result:
        ...
        print('{}发生异常'.format('创建子网'))
        logapp.info('{}发生异常'.format('创建子网'))
        exit(1)
    # 创建安全组
    secg_id = ''
    ...
    if cret_secg_ret:
        ...
        print('{}发生异常'.format('创建安全组'))
        logapp.info('{}发生异常'.format('创建安全组'))
        exit(1)
    # 创建安全组入方向规则
    if app1.cret_sec_inpolicy(secg_id):
        ...
        print('{}发生异常'.format('创建安全组入方向规则'))
        logapp.info('{}发生异常'.format('创建安全组入方向规则'))
        exit(1)
    # 创建安全组出方向规则
    if app1.cret_sec_outpolicy(secg_id):
        ...
        print('{}发生异常'.format('创建安全组出方向规则'))
        logapp.info('{}发生异常'.format('创建安全组出方向规则'))
        exit(1)
    # 查询支持的实例类型
    ...
    if cret_keypair_result:
        ...
        print('{}发生异常'.format('创建密钥对'))
        logapp.info('{}发生异常'.format('创建密钥对'))
        exit(1)

    app_id = user_config_dict.get('app_id')
    bucket_name = user_config_dict.get('bucket_name') + random_char

    cret_dict1['bucket_name'] = bucket_name


    if app1.create_bucket(app_id,bucket_name):
        print('bucket创建完成!,bucket名称:{}'.format(bucket_name))
        logapp.info('bucket创建完成!,bucket名称:{}'.format(bucket_name))
    else:
        print('{}发生异常'.format('bucket创建'))
        logapp.info('{}发生异常'.format('bucket创建'))
        exit(1)
...
def instance_oper(logapp):
    ...
    # 扫描本地系统获取系统信息
    OsType, Architecture, OsVersion,DataDisksSize,HostName= system_oper.sys_scan()
    num = 1
    while any([OsType,Architecture,OsVersion,DataDisksSize,HostName]):
        OsType, Architecture, OsVersion, DataDisksSize,HostName = system_oper.sys_scan()
        if all([OsType,Architecture,OsVersion,DataDisksSize,HostName]) or num > 5:
            break
        else:
            print('{}发生异常'.format('系统信息检测'))
            logapp.info('{}发生异常'.format('创建密钥对'))
            exit(1)
        time.sleep(5)
    ...
    # 获取地域公有镜像列表
    public_imglist = app2.des_imgid(imghub_type = 'PUBLIC_IMAGE').get('ImageSet')
    ...
    for img_content in public_imglist:
        if OsType == img_content.get('Platform'):
            if OsVersion == img_content.get('ImageName').split('.')[0].strip().split()[-1] and Architecture == img_content.get('Architecture'):
                # CentOS 7.3 64位
                ImageId = img_content.get('ImageId')
                cret_dict2['ImageId'] = ImageId
                break
        else:
            print('获取镜像ID失败, OsType:{},OsVersion:{},Architecture:{}',OsType,OsVersion,Architecture)
            exit(1)
...
def disk_oper(logapp):
    ...
    if system_oper.wirte_keypair(prikey_content, prikey_name, keydir):
        ...
        print('{}发生异常'.format('本地写入私钥'))
        logapp.info('{}发生异常'.format('本地写入私钥'))
        exit(1)
    ...
    if system_oper.dos2unix_shell(os_type):
        ...
        print('{}发生异常'.format('转换脚本'))
        logapp.info('{}发生异常'.format('转换脚本'))
        exit(1)
    # 传输文件
    ...
    if os_type == 'CentOS' or os_type == 'RedHatEnterpriseServer' or os_type == 'Debian':
        ...
        if my_sftp_disfunc and my_sftp_sysfunc:
            ...
            print('{}发生异常'.format('sftp 上传文件'))
            logapp.info('{}发生异常'.format('sftp 上传文件'))
            exit(1)
...
def rsycn_oper(logapp):
    ...
    if system_oper.rsync_file(publicip, keyfile,excfile):
        ...
        print('{}发生异常'.format('rsync文件'))
        logapp.info('{}发生异常'.format('rsync文件'))
        exit(1)
...
def bucket_oper(logapp):
    ...
    # bucket_name = user_config_dict.get('bucket_name') + random_char
    #
    # cret_dict6['bucket_name'] = bucket_name
...
def image_oper(logapp):
    ...
    bucket_name = cret_dict7.get('bucket_name')
...
def upload_img(logapp):
    ...
    while img_num <= 20:
        ...
        time.sleep(10)
        print('\r获取url导入镜像检测:{}'.format(img_num),end='')
...
def clean_env(logapp):
    ...
    bucket_name = cret_config_dict.get('bucket_name')
